...
LOOP:
	...
	res, err := hec.client.Do(req); if err != nil {
		return err
	}
...
func (hec *HEC) flushBuffer() {
	...
	if err := hec.makeRequest(hec.buffer); err != nil {
		fmt.Println("ERR while flushing", err)
	}	
	hec.buffer.Reset()
}
...
func (hec *HEC) WriteEvent(event *Event) {
	...
	if event.Index == "" { event.Index = hec.defaultIndex }
	if event.Source == "" { event.Source = hec.defaultSource }
	if event.SourceType == "" { event.SourceType = hec.defaultSourceType }
	if event.Host == "" { event.Host = hec.defaultHost }
